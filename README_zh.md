...
* **两步筛选掩码**：
    * **第一步：看区域属性**：根据每个掩码区域的像素面积 (Area)、总亮度或聚合值 (Intensity)，以及这两者的比率 (Ratio = Intensity/Area) 来做初步筛选。这些筛选可以单独开关，阈值也能自己定，相关浮点数值将以两位小数精度处理。
    * **第二步：看像素聚合值**：通过第一步的掩码，工具会进一步看区域里每个像素的聚合值（计算方式是 `100 - (灰度值 / 255.0) * 100.0`），只保留那些聚合值在指定范围内的像素。
* **结果可视化与预览**：
    * 筛选通过的掩码轮廓，连同它们的面积、总聚合值、比率这些信息（浮点数均保留两位小数），都会被画在原图上，然后保存成新的图片。
    * 提供“查看最后输出”功能，方便快速检查最近处理完成的图像。
* **参数配置管理**：
    * 可以一键**保存当前所有筛选参数和 SAM_HQ 模型参数**到 JSON 文件。
    * 可以方便地**加载之前保存的参数配置文件**，快速恢复工作设置。
* **数据导出清晰**：
    * 处理完成后，所有筛选通过的掩码的结构化数据（图像名、掩码索引、面积、强度、比率）可以一键导出为 `.txt` 文件，数值均保留两位小数。
* **输出文件好管理**：
    * 处理好的图片会保存在你选的输入文件夹的同级目录，并放在一个根据当前参数命名的子文件夹里。
    * 如果图片来自子文件夹，输出文件名会自动加上相对路径信息（路径分隔符会变成下划线），这样就算不同子文件夹里有同名图片，输出时也不会互相覆盖。
* **实时处理日志与进度**：
    * 界面上会显示详细的处理进度、总体进度条以及可能出现的任何问题。
* **系统资源监控**：
    * 在界面上实时显示 CPU 使用率、内存使用率。
    * 如果安装了相应库并有 NVIDIA 显卡，还会显示 GPU 使用率和显存使用率。
...
* 在打开的终端中，使用以下 `pip install` 命令来安装各个包：
    ...
    * **安装其他核心依赖包:**
        ...
        * **关于 `pycocotools` 的小提示**：在 Windows 上直接用 `pip install pycocotools` 容易碰到编译错误。如果真遇上了：
            * 可以试试 `pip install pycocotools-windows`。
...
* **Save Config (保存配置)**: 点击此按钮可以将当前界面上所有的“阶段 1 筛选参数”、“阶段 2 筛选参数”以及“SAM_HQ 自动掩码生成器参数”保存到一个 JSON (`.json`) 文件中。方便下次使用时快速加载。
* **Load Config (加载配置)**: 点击此按钮可以选择之前保存的 JSON 配置文件，程序会自动将文件中的参数设置回界面对应的控件上。

### 3. Phase 1: Region Property Filters (第一步筛选：看区域属性)

SAM_HQ 会生成很多初始的掩码区域，这里的筛选器就是针对这些区域的。相关浮点数值（如强度、比率的阈值）在界面上和计算时均以两位小数精度处理。

* **Min Area (A) Threshold (最小面积阈值)**: 一个区域至少要有多少像素才算数。默认：199。
* **Min Intensity (I) Threshold (最小总聚合值阈值)**: 一个区域里所有像素的聚合值（算法是 `100 - (灰度值/255)*100`）加起来的总和，不能低于这个数。默认：0.00。
* **Min Ratio (R=I/A) Threshold (最小比率阈值)**: 上面那个总聚合值 (Intensity) 除以面积 (Area) 得到的比率，不能低于这个数。默认：44.00。
* **Enable Phase 1 Filters (启用第一步筛选)**:
    * **Area (A)**: 勾上才用面积筛选。
    * **Intensity/Ratio (I/R)**: 勾上才用总聚合值和比率筛选。

### 4. Phase 2: Pixel Aggregate Filters (第二步筛选：看像素聚合值)
...
### 5. SAM_HQ Auto Mask Generator Parameters (SAM_HQ 自动找图参数)
这部分是告诉电脑怎么更聪明、更细致地在图片上自动找出各种东西（我们叫它“生成掩码”）。你可以把这些参数想象成调节一台超级相机的各种旋钮和按钮，调好了就能拍出效果更好的“分割图”。相关浮点数值参数在界面上以两位小数（部分为三位，如裁剪重叠比率）显示。
...
* **Pred IoU Thresh (预测效果好坏的门槛)**: 电脑找到一个东西后，会给它打个分，看看这个东西找得准不准。这个分数叫 IoU。这个参数就是设一个门槛，只有分数高于这个门槛的，才认为是找得不错的。默认是 0.86。
* **Stability Score Thresh (找得稳不稳的门槛)**: 有时候电脑找东西会“手抖”，同一个东西可能一会儿看像这个，一会儿看像那个。这个参数也是设一个门槛，看电脑找这个东西稳不稳定，越稳定越好。默认是 0.92。
* **Stability Score Offset (稳定分微调)**: 这是上面那个“稳定分”的一个小调整参数。默认是 1.00。
* **Box NMS Thresh (去掉重复框框的门槛)**: 电脑找东西的时候，可能会对同一个东西画好几个框框。这个参数就是告诉电脑，如果好几个框框都圈着差不多的东西，就把多余的去掉，只留一个最好的。默认是 0.70。
* **Crop N Layers (把大图切成几块看)**: 如果图片特别大，电脑可能会把它切成几小块，一块一块地看，看得更仔细。这个参数就是说要切成几层来看。默认是 1 层。
* **Crop NMS Thresh (切块后去重门槛)**: 上面说到切块看，那不同小块里可能也找到了重复的东西。这个参数跟“去掉重复框框的门槛”类似，也是用来去掉这些重复的。默认是 0.70。
* **Crop Overlap Ratio (切块时边上留多少重叠)**: 把图片切成小块时，为了不漏掉边缘的东西，相邻的小块之间会有一点点重叠。这个参数就是说重叠部分占多大比例。默认是 0.341 (SAM-HQ 默认 `512/1500`)。
...
* **Start Processing All Images (开始处理所有图片)**: 输入文件夹和模型文件都选好了之后，点这个就开始处理找到的所有图片。处理过程中会显示总体进度条。
...
* **Export All Values (导出全部数值)**: 处理完成后，点击此按钮可以将所有通过筛选的掩码的结构化数据（图像名、掩码索引、面积、强度、比率）导出为一个 `.txt` 文件。强度和比率数值会保留两位小数。
* **View Last Output (查看最后输出)**: 点击此按钮会尝试使用系统默认的图片查看器打开最后一张成功处理并保存的图片，方便快速预览。

### 7. System Monitor (系统监控区)

* 这个区域会（如果相关库已安装）周期性地显示以下信息：
    * **CPU Usage (CPU 使用率)**: 当前 CPU 的总体使用百分比。
    * **RAM Usage (内存使用率)**: 当前内存的使用百分比及具体用量。
    * **GPU Usage (GPU 使用率)**: 如果有 NVIDIA 显卡且 `pynvml` 库工作正常，显示第一个 GPU 的核心使用率。
    * **VRAM Usage (显存使用率)**: 如果有 NVIDIA 显卡且 `pynvml` 库工作正常，显示第一个 GPU 的显存使用百分比及具体用量。
* 如果相应的监控库（`psutil` 或 `pynvml`）未安装或初始化失败，对应项会显示 "N/A" 或错误提示。

### 8. Processing Log (处理日志区)

* 这个区域会告诉你程序正在干嘛、处理到哪儿了，以及有没有什么警告或者错误。包括系统监控库的加载状态。

## 一些小提醒

* 第一次加载模型，或者处理特别大的图片时，可能会花点时间，耐心等等。
* 确保你电脑硬盘有足够的空间来保存处理完的图片和导出的数据文件。
